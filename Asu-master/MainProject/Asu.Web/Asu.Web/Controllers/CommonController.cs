...
namespace Asu.Web.Controllers
{
    public class CommonController : BasePublicController
    {
        ...
        private readonly ILogger _logger;   //WC
        ...
        public CommonController(IVehicleService vehicleService,
        ...
        public ActionResult PageNotFound()
        {
            ...
            if (url != null)
            {
                this._logger.InsertLog(LogLevel.Error, "404", url.ToString());
            }*/
        ...
        //header links
        [ChildActionOnly]
        public ActionResult MobileMenuLinks()
        {
            var customer = _workContext.CurrentCustomer;
        ...
        public ActionResult AdminHeaderLinks()
        {
            ...
            var model = new AdminHeaderLinksModel
            {
                ...
                DisplayServerDetails = customer.IsDeveloper()
            };
        ...
        //garage options
        [ChildActionOnly]
        public ActionResult HeaderGarageOption()
        {
            var customer = _workContext.CurrentCustomer;

            GarageVehicleModel firstVehicle = null;
            var vehicleGarageList = new List<GarageVehicleModel>();

            if (customer.VehicleGarage.Count > 0)
            {
                foreach (var vg in customer.VehicleGarage.AsEnumerable().OrderByDescending(x => x.IsMain))
                {
                    vehicleGarageList.Add(new GarageVehicleModel { Vehicle = vg.Vehicle, IsMain = vg.IsMain, VehicleName = string.Format("{0} {1} {2} {3}", vg.Vehicle.BaseVehicle.YearId, vg.Vehicle.BaseVehicle.Make.Name, vg.Vehicle.BaseVehicle.Model.Name, vg.Vehicle.SubModel.Name) });
                }

                firstVehicle = vehicleGarageList.FirstOrDefault(x => x.IsMain);
                if (firstVehicle != null)
                {
                    this._vehicleService.SetVehicleToCookies(firstVehicle.Vehicle.BaseVehicle.YearId, firstVehicle.Vehicle.BaseVehicle.MakeId, firstVehicle.Vehicle.BaseVehicle.ModelId, firstVehicle.Vehicle.SubModelId, false);
                }
            }

            var model = new HeaderGarageOptionModel
            {
                VehicleGarage = vehicleGarageList,
            };
        ...
        [HttpPost]
        public JsonResult SetVehicle(int yearId, int makeId, int modelId, int subModelId)
        {
            var vehicle = this._vehicleService.GetVehicle(yearId, makeId, modelId, subModelId);

            this._vehicleService.AddVehicleToCustomerGarage(vehicle.Id, _workContext.CurrentCustomer.Id, true);
            this._vehicleService.SetVehicleToCookies(yearId, makeId, modelId, subModelId, false);
            return null;
        }

        //[HttpPost]
        //public void ClearVehicles(int vehicleId)
        //{
        //    Console.WriteLine("Clear" + st);
        //    //return RedirectToAction("Index", "HomeController");
        //    //this._vehicleService.ClearVehicleCookies();
        //    //this._vehicleService.ClearCustomerGarage(_workContext.CurrentCustomer.Id);
        ...
        public ActionResult ContactUs()
        {
            var model = new ContactUsModel
            {
                ...
                DisplayCaptcha = false // WC. _captchaSettings.Enabled && _captchaSettings.ShowOnContactUsPage
            };
        ...
        {
            // WC. use a little trick with hidden field 
            var isBotSender = !string.IsNullOrWhiteSpace(model.Comment);
            if (isBotSender)
            {
                //validate CAPTCHA
            ...
            if (ModelState.IsValid && !isBotSender)
            {
            ...
            model.DisplayCaptcha = _captchaSettings.Enabled && _captchaSettings.ShowOnContactUsPage && isBotSender;
            return View(model);
        }
        ...
        public ActionResult RobotsTextFile()
        {
            try
            {
                var content = System.IO.File.ReadAllText(seoSettings.RobotsTxtPath);
                if (string.IsNullOrEmpty(content))
                {
                    this._logger.InsertLog(LogLevel.Error, $"Error when reading robots.txt from {this._storeContext.CurrentStore.CompanyName} store.");
                }
            ...
            {
                _logger.InsertLog(LogLevel.Error, e.Message, e.StackTrace);
                _logger.InsertLog(LogLevel.Error, $"Error when reading robots.txt from {this._storeContext.CurrentStore.CompanyName} store.");
            }
            ...
            //    sb.AppendFormat("Sitemap: {0}sitemap/sitemap_category.xml", _storeContext.CurrentStore.Url);
            //    sb.Append(newLine);
            //    sb.AppendFormat("Sitemap: {0}sitemap/sitemap_manufacturer.xml", _storeContext.CurrentStore.Url);
            //    sb.Append(newLine);
            //    sb.AppendFormat("Sitemap: {0}sitemap/sitemap_category_manufacturer.xml", _storeContext.CurrentStore.Url);
            //    sb.Append(newLine);
            //    sb.AppendFormat("Sitemap: {0}sitemap/sitemap_manufacturer_category.xml", _storeContext.CurrentStore.Url);
            //    sb.Append(newLine);
            //    sb.AppendFormat("Sitemap: {0}sitemap/sitemap_products_index.xml", _storeContext.CurrentStore.Url);
            //    sb.Append(newLine);
            //    sb.AppendFormat("Sitemap: {0}sitemap/sitemap_vehicle_accessories.xml", _storeContext.CurrentStore.Url);
            //    sb.Append(newLine);
            //    sb.AppendFormat("Sitemap: {0}sitemap/sitemap_vehicle_categories_index.xml", _storeContext.CurrentStore.Url);
            //    sb.Append(newLine);
            //    sb.AppendFormat("Sitemap: {0}sitemap/sitemap_vehicle_categories_001.xml", _storeContext.CurrentStore.Url);
            //    sb.Append(newLine);
            //}
