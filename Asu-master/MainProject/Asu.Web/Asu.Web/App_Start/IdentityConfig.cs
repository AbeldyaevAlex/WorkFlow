...
namespace Asu.Web
{
    ...
    {
        public int RequredLenght { get; set; }
        public CustomPasswordValidator(int lenght)
        {
            RequredLenght = lenght;
        }
        public Task<IdentityResult> ValidateAsync(string item)
        {
            if (String.IsNullOrEmpty(item) || item.Length < RequredLenght)
            {
                return Task.FromResult(IdentityResult.Failed(String.Format("Минимальная длина пароля равна {0}", RequredLenght)));
            }
            //var pattern = (@"^(?=.{8, 16}$)(?=.*?[a-z])(?=.*?[A-Z])(?=.*?[0-9]).*$");
            var pattern = @"^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])";
            if (!Regex.IsMatch(item, pattern))
            {
                return Task.FromResult(IdentityResult.Failed(String.Format("Пароль должен содержать латинские буквы цифры и знаки препинания")));
            }
