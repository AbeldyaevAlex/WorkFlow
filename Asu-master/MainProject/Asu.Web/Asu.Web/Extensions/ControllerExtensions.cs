...
namespace Asu.Web.Extensions
{
    ...
    public static class ControllerExtensions
    {
            ...
            int productId)
        {
            ...
            string cacheKey = string.Format(ModelCacheEventConsumer.PRODUCT_SPECS_MODEL_KEY, productId, workContext.WorkingLanguage.Id);
            Func<ProductSpecificationAttribute, string> getSpecificationAttributeValue = (psa) =>
            {
                var value = string.Empty;
                switch (psa.AttributeType)
                {
                    case SpecificationAttributeType.Option:
                        value =  HttpUtility.HtmlEncode(psa.SpecificationAttributeOption.GetLocalized(x => x.Name));
                        break;
                    case SpecificationAttributeType.CustomText:
                        value =  HttpUtility.HtmlEncode(psa.CustomValue);
                        break;
                    case SpecificationAttributeType.CustomHtmlText:
                        value = psa.CustomValue;
                        break;
                    case SpecificationAttributeType.Hyperlink:
                        value = string.Format("<a href='{0}' target='_blank'>{0}</a>", psa.CustomValue);
                        break;
                    default:
                        break;
                }
            ...
            {
                var productSpecificationAttributes = specificationAttributeService.GetProductSpecificationAttributesByProductId(productId, null, true);
                var model = productSpecificationAttributes
                    .Select(psa => {
                        var descriptions = psa.SpecificationAttributeOption.SpecificationAttribute.Descriptors;
                        var m = new ProductSpecificationModel
                        {
                            SpecificationAttributeId = psa.SpecificationAttributeOption.SpecificationAttributeId,
                            SpecificationAttributeName = psa.SpecificationAttributeOption.SpecificationAttribute.GetLocalized(x => x.Name),
                            ValueRaw = getSpecificationAttributeValue(psa),
                            SpecificationAttributeDescriptions = productSpecificationAttributes
                            .Join(descriptions, a => a.SpecificationAttributeOption.SpecificationAttributeId, b => b.DescriptorAttributeId, (a, b) => 
                            new
                            {
                                ProductSpecificationAttribute = a,
                                b.DescriptorAttributeId,
                                b.DisplayName,
                                b.DisplayOrder
                            })
                            .OrderBy(psad => psad.DisplayOrder)
                            .Select(psad => new ProductSpecificationModel
                            {
                                SpecificationAttributeId = psad.DescriptorAttributeId,
                                SpecificationAttributeName = string.IsNullOrEmpty(psad.DisplayName) 
                                    ? psad.ProductSpecificationAttribute.SpecificationAttributeOption.SpecificationAttribute.Name 
                                    : psad.DisplayName,
                                ValueRaw = getSpecificationAttributeValue(psad.ProductSpecificationAttribute)
                            })
                            .ToList()
                        };
            ...
            foreach (var product in products)
            {
                                    ...
                                    if (!product.CustomerEntersPrice)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            priceModel.DisplayTaxShippingInfo = catalogSettings.DisplayTaxShippingInfoProductBoxes
                                                ...
                                                !shippingService.IsFreeShipping(product, storeId);
                                        }
                                    }
                                }
                                else
                ...
                if (prepareSpecificationAttributes)
                {
                    model.SpecificationAttributeModels = PrepareProductSpecificationModel(controller, workContext,
                         specificationAttributeService, cacheManager, product.Id);
                }
