    ...
    public partial class GenericPathRoute : LocalizedRoute
    {
        ...
        public override RouteData GetRouteData(HttpContextBase httpContext)
        {
            ...
            if (data != null && DataSettingsHelper.DatabaseIsInstalled())
            {
                ...
                if (urlRecord == null)
                {
                    urlRecord = urlRecordService.GetRedirectBySlugCached(slug);
                    if (urlRecord != null)
                    {
                        ...
                        response.RedirectLocation = $"{webHelper.GetStoreLocation(false)}{urlRecord.Slug}"; // 
                        response.End();
                        return null;
                    }
                ...
                var slugForCurrentLanguage = urlRecord.AdditionalEntityId.HasValue ? SeoExtensions.GetSeName(urlRecord.EntityId, urlRecord.AdditionalEntityId.Value, urlRecord.EntityName, workContext.WorkingLanguage.Id)
                    : SeoExtensions.GetSeName(urlRecord.EntityId, urlRecord.EntityName, workContext.WorkingLanguage.Id);
                ...
                switch (urlRecord.EntityName.ToLowerInvariant())
                {
                    ...
                    case "category":
                        {
                            data.Values["controller"] = "Vehicle";
                    ...
                    case "categorymanufacturer":
                        {
                            data.Values["controller"] = "Vehicle";
                            data.Values["action"] = "CategoryManufacturer";
                            data.Values["categoryid"] = urlRecord.EntityId;
                            data.Values["manufacturerid"] = urlRecord.AdditionalEntityId.HasValue ? urlRecord.AdditionalEntityId.Value : 0;
                            data.Values["SeName"] = urlRecord.Slug;
                        }
                        break;
                    case "manufacturercategory":
                        {
                            data.Values["controller"] = "Vehicle";
                            data.Values["action"] = "ManufacturerCategory";
                            data.Values["manufacturerid"] = urlRecord.EntityId;
                            data.Values["categoryid"] = urlRecord.AdditionalEntityId.HasValue ? urlRecord.AdditionalEntityId.Value : 0;
                            data.Values["SeName"] = urlRecord.Slug;
                        }
                        break;
                    default:
