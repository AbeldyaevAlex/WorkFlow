...
namespace Asu.Data.Mapping.Catalog
{
    public partial class ProductMap : NopEntityTypeConfiguration<Product>
    {
        public ProductMap()
        {
            ...
            this.HasMany(p => p.FakeReviews)
                .WithMany(pt => pt.Products)
                .Map(m => m.ToTable("WCS_Product_FakeReview_Mapping"));

            this.HasMany(p => p.CompetitorPrices)
                .WithRequired(cp => cp.Product);

            this.HasOptional(p => p.ProductExtra).WithRequired(pe => pe.Product);

            this.HasMany(p => p.ProductVehicleFitments).WithOptional().HasForeignKey(p => p.ProductId);
