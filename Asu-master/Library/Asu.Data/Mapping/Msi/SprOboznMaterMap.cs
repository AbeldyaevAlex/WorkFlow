using Asu.Core.Domain.Msi;

namespace Asu.Data.Mapping.Msi
{
    public partial class SprOboznMaterMap : NopEntityTypeConfiguration<Spr_obozn_mater>
    {
        public SprOboznMaterMap()
        {
            this.ToTable("Spr_obozn_mater");
            this.HasKey(l => l.Id);
            this.Property(mas => mas.Mas1sh).HasPrecision(38, 7);

            this.HasRequired(a => a.Customer)
                .WithMany()
                .HasForeignKey(x => x.CustomerId)
                .WillCascadeOnDelete(false);

            this.HasRequired(a => a.DocumentStatus)
                .WithMany()
                .HasForeignKey(x => x.DocumentStatusId)
                .WillCascadeOnDelete(false);

            this.HasRequired(a => a.Spr_pvi)
               .WithMany()
               .HasForeignKey(x => x.PviId)
               .WillCascadeOnDelete(false);
        }
    }
}
