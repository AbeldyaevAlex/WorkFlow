...
namespace Asu.Services.Customers
{
    /// <summary>
    /// Customer service
    /// </summary>
    public partial class CustomerService : ICustomerService
    {
        ...
        public virtual List<CustomerRole> GetAllRoles()
        {
            var query = _customerRoleRepository.Table;
            var customersRoles = query.ToList();
               ...
               new TransactionOptions { IsolationLevel = IsolationLevel.ReadCommitted }))
                {
                    ...
                    {
                        using (var cmd = new SqlCommand("GetNextCustomer", connection))
                        {
                            ...
                            if (!int.TryParse(cmd.Parameters[0].Value.ToString(), out customerId))
                            {
                                throw new NopException("Customer ID could not be parsed");
                            }
                        }
                    }
            ...
            bool clearPaymentMethod = true)
        {
            ...
            if (clearCouponCodes && clearCheckoutAttributes && clearRewardPoints && clearShippingMethod && clearPaymentMethod)
            {
                // improve performance when checkout
                this._dbContext.ExecuteSqlCommand(@"EXEC [WC_ResetCheckoutData] @CustomerId", false, null, new System.Data.SqlClient.SqlParameter("@CustomerId", customer.Id));
            }
            else
            {
                //clear entered coupon codes
            ...
            DateTime? createdToUtc, bool onlyWithoutShoppingCart, int maxNumberOfRecordsToDelete)
        {
            ...
            return this._dbContext.ExecuteSqlCommand(@"EXEC [WC_DeleteGuestCustomers] @count, @createdToUtc, @roleId", 
                false, 
                null,
                new System.Data.SqlClient.SqlParameter("@count", maxNumberOfRecordsToDelete),
                new System.Data.SqlClient.SqlParameter("@createdToUtc", createdToUtc),
                new System.Data.SqlClient.SqlParameter("@roleId", guestRole.Id));
