...
namespace Asu.Services.Catalog
{
    ...
    public static class TierPriceExtensions
    {
            ...
            Customer customer)
        {
            ...
            // WC: include only tier prices by customer role
            if (result.Any(r => r.CustomerRoleId.HasValue))
            {
                result = result.Where(r => r.CustomerRoleId.HasValue).Select(r => r).ToList();
            }
