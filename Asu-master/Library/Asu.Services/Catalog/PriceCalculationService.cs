...
using Asu.Services.Catalog.Cache;
...
namespace Asu.Services.Catalog
{
    ...
    public partial class PriceCalculationService : IPriceCalculationService
    {
        ...
        public PriceCalculationService(IWorkContext workContext,
            ...
            IManufacturerService manufacturerService,   // WC
            ...
            ISalesQuoteService salesQuoteService)
        {
            ...
            this.customHelper = customHelper; // WC
            ...
            Customer customer)
        {
            ...
            if (_discountService.GetAllDiscounts(DiscountType.AssignedToBrands).Any())
            {
                var productBrands = _manufacturerService.GetProductManufacturersByProductId(product.Id);
                if (productBrands != null)
                {
                    foreach (var productBrand in productBrands)
                    {
                        var brand = productBrand.Manufacturer;

                        if (brand.HasDiscountsApplied)
                        {
                            //we use this property ("HasDiscountsApplied") for performance optimziation to avoid unnecessary database calls
                            var brandDiscounts = brand.AppliedDiscounts;
                            foreach (var discount in brandDiscounts)
                            {
                                if (_discountService.IsDiscountValid(discount, customer) &&
                                    discount.DiscountType == DiscountType.AssignedToBrands &&
                                    !allowedDiscounts.ContainsDiscount(discount))
                                    allowedDiscounts.Add(discount);
                            }
                        }
            ...
            out Discount appliedDiscount)
        {
            ...
            #region WC - Admin price override

            var isQuote = this.salesQuoteService.GetQuoteIdFromCookie() > 0;
            if (this._workContext.CurrentCustomer.IsAdmin() || isQuote)
            {
                var adminPrice = _workContext.CurrentCustomer.GetAttribute<decimal?>(string.Format("AdminProductPrice-{0}", shoppingCartItem.ProductId), _storeContext.CurrentStore.Id);
                if (adminPrice.HasValue)
                {
                    unitPrice = adminPrice.Value;
                }
            }
