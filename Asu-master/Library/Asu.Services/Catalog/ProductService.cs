...
namespace Asu.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        private const string PRODUCTS_IDS = "Available.Products";
        private readonly DateTime resetCacheTime = DateTime.Now.AddDays(1);
        ...
        private readonly IRepository<ProductGroupClubPrice> _productGroupClubPriceRepository;
        ...
        private readonly IRepository<ProductInventoryManageMethod> inventoryManageMethodRepository;
        private readonly IDbHelper dbHelper;
        private const string OVERRIDED_MANAGE_INVENTORY_METHODS_KEY = "WC.OverridedManageInventoryMethods-{0}";
        private readonly IRepository<BestSellerProduct> bestSellerProductRepository;
        ...
        public ProductService(ICacheManager cacheManager,
            ...
            IRepository<ProductGroupClubPrice> productGroupClubPriceRepository,
            ...
            IRepository<ProductInventoryManageMethod> inventoryManageMethodRepository,
            IDbHelper dbHelper,
            IRepository<BestSellerProduct> bestSellerProductRepository)
        {
            ...
            this._productGroupClubPriceRepository = productGroupClubPriceRepository;
            ...
            this.inventoryManageMethodRepository = inventoryManageMethodRepository;
        ...
        public virtual IList<Product> GetProductsByIds(int[] productIds, bool sortByCollection)
        {
            if (productIds == null || productIds.Length == 0)
                return null;

            var query = from p in _productRepository.Table
                        join pid in productIds on p.Id equals pid
                        select p;

            var products = query.ToList();

            if (sortByCollection == false)
            {
                return products;
            }

            //sort by passed identifiers
            var sortedProducts = products.OrderBy(x => { return Array.IndexOf(productIds, x.Id); }).ToList();

            return sortedProducts;
        }


        public virtual IQueryable<Product> GetProductsByIds(int[] productIds)
        {
            if (productIds == null || productIds.Length == 0)
                return null;
        ...
        public virtual IQueryable<int> GetProductIds(bool showHidden = false)
        {
            //var productIdsCache = _cacheManager.Get(PRODUCTS_IDS,  (resetCacheTime - DateTime.Now).Minutes, () =>
            var productIdsCache = _cacheManager.Get(PRODUCTS_IDS, 3600, () =>
            {
                var products = this._productRepository.TableNoTracking;

                if (!showHidden)
                {
                    products = products.Where(a => a.Published && !a.Deleted
                    && (a.StockQuantity > 0 || a.ProductExtra.IsShippingFromManufacturer)
                    && !a.DisableBuyButton && !a.CallForPrice);
                }

                if (!_catalogSettings.IgnoreStoreLimitations)
                {
                    return products.Join(this._storeMappingRepository.TableNoTracking, a => a.Id, b => b.EntityId, (a, b) => new { Product = a, StoreMapping = b })
                                .Where(c => c.StoreMapping.StoreId == _storeContext.CurrentStore.Id && c.StoreMapping.EntityName == "Product" || !c.Product.LimitedToStores)
                                .Select(d => d.Product.Id);
                }

                return products.Select(d => d.Id);
            });
        ...
        public virtual void UpdateProduct(Product product)
        {
            ...
            //event notification
            //_eventPublisher.EntityUpdated(product);   // WC. Commented to improve performance
        }
            ...
            else
            {
                    ...
                    query = query.Where(p =>
                                        ...
                                        ((!p.SpecialPrice.HasValue ||
                                          ((p.SpecialPriceStartDateTimeUtc.HasValue &&
                                           ...
                                           (p.SpecialPriceEndDateTimeUtc.HasValue &&
                                            p.SpecialPriceEndDateTimeUtc.Value < nowUtc))) &&
                                         (priceCalculationService.GetFinalPrice(p, this._workContext.CurrentCustomer, 0M, true, 1) >= priceMin.Value)));
                }
                    ...
                    query = query.Where(p =>
                                        ...
                                        ((!p.SpecialPrice.HasValue ||
                                          ((p.SpecialPriceStartDateTimeUtc.HasValue &&
                                           ...
                                           (p.SpecialPriceEndDateTimeUtc.HasValue &&
                                            p.SpecialPriceEndDateTimeUtc.Value < nowUtc))) &&
                                         (priceCalculationService.GetFinalPrice(p, this._workContext.CurrentCustomer, 0M, true, 1) <= priceMax.Value)));
                }
                if (!showHidden)
                {
                    //available dates
                ...
                if (warehouseId > 0)
                {
                    ...
                    query = query.Where(p =>
                        ...
                        ((int)this.GetInventoryManageMethod(p, storeId) == manageStockInventoryMethodId &&
                ...
                var priceCalculationSrvice = EngineContext.Current.Resolve<IPriceCalculationService>();
                var customer = this._workContext.CurrentCustomer;
                query.ToList().ForEach(p => p.Price = priceCalculationSrvice.GetFinalPrice(p, customer));
        ...
        /// <param name="pageIndex">Page index</param>
        /// <param name="pageSize">Page size</param>
        /// <param name="includedCategoryIds">Category identifiers</param>
        /// <param name="showHidden">A value indicating whether to show hidden records</param>
        /// <param name="excludedCategoryIds">Without category identifiers</param>
        /// <param name="includedManufacturerIds">Manufacturer identifiers</param>
        /// <param name="excludedManufacturerIds">Without manufacturer identifiers</param>
        /// <returns>Products</returns>
        public virtual IPagedList<Product> AdvanceSearchProducts(int pageIndex = 0,
            int pageSize = int.MaxValue,
            IList<int> includedCategoryIds = null,
            IList<int> excludedCategoryIds = null,
            IList<int> includedManufacturerIds = null,
            IList<int> excludedManufacturerIds = null)
        {
            //validate "categoryIds" parameter
            if (includedCategoryIds != null && includedCategoryIds.Contains(0))
                includedCategoryIds.Remove(0);

            //Access control list. Allowed customer roles
            var allowedCustomerRolesIds = _workContext.CurrentCustomer.CustomerRoles
                .Where(cr => cr.Active).Select(cr => cr.Id).ToList();

            //validate "excludedCategoryIds" parameter
            if (excludedCategoryIds != null && excludedCategoryIds.Contains(0))
                excludedCategoryIds.Remove(0);

            //validate "includedManufacturerIds" parameter
            if (includedManufacturerIds != null && includedManufacturerIds.Contains(0))
                includedManufacturerIds.Remove(0);

            //validate "excludedManufacturerIds" parameter
            if (excludedManufacturerIds != null && excludedManufacturerIds.Contains(0))
                excludedManufacturerIds.Remove(0);
            ...
            if (_commonSettings.UseStoredProceduresIfSupported && _dataProvider.StoredProceduredSupported)
            {
                ...
                {
                    for (int i = 0; i < includedCategoryIds.Count; i++)
                    {
                        commaSeparatedCategoryIds += includedCategoryIds[i].ToString();
                        if (i != includedCategoryIds.Count - 1)
                        {
                            commaSeparatedCategoryIds += ",";
                        }
                    }
                ...
                //pass excluded category identifiers as comma-delimited string
                string commaSeparatedExcludedCategoryIds = "";
                if (excludedCategoryIds != null)
                {
                    for (int i = 0; i < excludedCategoryIds.Count; i++)
                    {
                        commaSeparatedExcludedCategoryIds += excludedCategoryIds[i].ToString();
                        if (i != excludedCategoryIds.Count - 1)
                        {
                            commaSeparatedExcludedCategoryIds += ",";
                        }
                    }
                }

                //pass included manufacturer identifiers as comma-delimited string
                string commaSeparatedIncludedManufacturerIds = "";
                if (includedManufacturerIds != null)
                {
                    for (int i = 0; i < includedManufacturerIds.Count; i++)
                    {
                        commaSeparatedIncludedManufacturerIds += includedManufacturerIds[i].ToString();
                        if (i != includedManufacturerIds.Count - 1)
                        {
                            commaSeparatedIncludedManufacturerIds += ",";
                        }
                    }
                }

                //pass excluded manufacturer identifiers as comma-delimited string
                string commaSeparatedExcludedManufacturerIds = "";
                if (excludedManufacturerIds != null)
                {
                    for (int i = 0; i < excludedManufacturerIds.Count; i++)
                    {
                        commaSeparatedExcludedManufacturerIds += excludedManufacturerIds[i].ToString();
                        if (i != excludedManufacturerIds.Count - 1)
                        {
                            commaSeparatedExcludedManufacturerIds += ",";
                        }
                    }
                }
                ...
                //prepare parameters
                var pInCategoryIds = _dataProvider.GetParameter();
                pInCategoryIds.ParameterName = "InCategoryIds";
                pInCategoryIds.Value = commaSeparatedCategoryIds != null ? (object)commaSeparatedCategoryIds : DBNull.Value;
                pInCategoryIds.DbType = DbType.String;

                var pExCategoryIds = _dataProvider.GetParameter();
                pExCategoryIds.ParameterName = "ExCategoryIds";
                pExCategoryIds.Value = commaSeparatedExcludedCategoryIds != null ? (object)commaSeparatedExcludedCategoryIds : DBNull.Value;
                pExCategoryIds.DbType = DbType.String;

                var pInManufacturerIds = _dataProvider.GetParameter();
                pInManufacturerIds.ParameterName = "InManufacturerIds";
                pInManufacturerIds.Value = commaSeparatedIncludedManufacturerIds != null ? (object)commaSeparatedIncludedManufacturerIds : DBNull.Value;
                pInManufacturerIds.DbType = DbType.String;

                var pExManufacturerIds = _dataProvider.GetParameter();
                pExManufacturerIds.ParameterName = "ExManufacturerIds";
                pExManufacturerIds.Value = commaSeparatedExcludedManufacturerIds != null ? (object)commaSeparatedExcludedManufacturerIds : DBNull.Value;
                pExManufacturerIds.DbType = DbType.String;
            ...
            else
            {
                ...
                //category filtering
                if (includedCategoryIds != null && includedCategoryIds.Count > 0)
                {
                    query = from p in query
                            from pc in p.ProductCategories.Where(pc => includedCategoryIds.Contains(pc.CategoryId))
                            select p;
                }

                //excluded category filtering
                if (excludedCategoryIds != null && excludedCategoryIds.Count > 0)
                {
                    query = from p in query
                            from pc in p.ProductCategories.Where(pc => !excludedCategoryIds.Contains(pc.CategoryId))
                            select p;
                }

                //included manufacturer filtering
                if (includedManufacturerIds != null && includedManufacturerIds.Count > 0)
                {
                    query = from p in query
                            from pc in p.ProductManufacturers.Where(pc => includedManufacturerIds.Contains(pc.ManufacturerId))
                            select p;
                }

                //excluded manufacturer filtering
                if (excludedManufacturerIds != null && excludedManufacturerIds.Count > 0)
                {
                    query = from p in query
                            from pc in p.ProductManufacturers.Where(pc => !excludedManufacturerIds.Contains(pc.ManufacturerId))
                            select p;
                }
        ...
        {
            var storeId = this._storeContext.CurrentStore.Id;
            //Track inventory for product
            var query1 = from p in this._productRepository.Table
                         orderby p.MinStockQuantity
                         where !p.Deleted &&
                        (int)this.GetInventoryManageMethod(p, storeId) == (int)ManageInventoryMethod.ManageStock &&
            ...
            var query2 = from p in this._productRepository.Table
                         from pvac in p.ProductVariantAttributeCombinations
                         where !p.Deleted &&
                        (int)this.GetInventoryManageMethod(p, storeId) == (int)ManageInventoryMethod.ManageStockByAttributes &&
        ...
        public virtual void AdjustInventory(Product product, int quantityToChange, string attributesXml = "")
        {
            ...
            if (this.GetInventoryManageMethod(product, storeId) == ManageInventoryMethod.ManageStockByAttributes)
            {
        ...
        public virtual void BookReservedInventory(Product product, int warehouseId, int quantity)
        {
            ...
            var storeId = EngineContext.Current.Resolve<IStoreContext>().CurrentStore.Id;
            if (this.GetInventoryManageMethod(product, storeId) != ManageInventoryMethod.ManageStock)
                return;
            if (!product.UseMultipleWarehouses)
                return;
            ...
            foreach (var sci in cart)
            {
                ...
                foreach (var crossSell in crossSells)
                {
                        ...
                        !cartProductIds.Contains(crossSell.ProductId2))
                    {
                        ...
                        //validate product
                        if (productToAdd == null || productToAdd.Deleted || !productToAdd.Published || (productToAdd.StockQuantity == 0 && !productToAdd.ProductExtra.IsShippingFromManufacturer))
                            continue;
        ...
        #region Product Group Club Price

        public virtual ProductGroupClubPrice GetProductGroupClubPriceById(int productGroupClubPriceId)
        {
            if (productGroupClubPriceId == 0)
                return null;

            var query = from pp in _productGroupClubPriceRepository.Table
                        where pp.ProductId == productGroupClubPriceId
                        select pp;
            ProductGroupClubPrice productGroupClubPrices = query.FirstOrDefault();
            return productGroupClubPrices;

            //return _productGroupClubPriceRepository.GetById(ProductGroupClubPriceId);
        }
        ...
        public ManageInventoryMethod GetInventoryManageMethod(Product product, int storeId)
        {
            var productId = product.Id;
            var method = (from m in this.inventoryManageMethodRepository.TableNoTracking
                          where m.ProductId == productId && m.StoreId == storeId
                          select m).FirstOrDefault();

            return method != null ? (ManageInventoryMethod)method.ManageInventoryMethodId : product.ManageInventoryMethod;
        }

        public ManageInventoryMethod GetInventoryManageMethod(int productId, ManageInventoryMethod manageInventoryMethod, int storeId)
        {
            var method = (from m in this.inventoryManageMethodRepository.TableNoTracking
                          where m.ProductId == productId && m.StoreId == storeId
                          select m).FirstOrDefault();

            return method != null ? (ManageInventoryMethod)method.ManageInventoryMethodId : manageInventoryMethod;
        }

        public int GetOutstockOrderAvgShipLeadTime(int productId, int channelId)
        {
            return dbHelper.CallStoredProcedure<int>("GetOutstockOrderAvgShipLeadTime", productId, channelId);
        }

        public IList<Product> GetBestSellerProducts(int takeCount, bool showHidden = false)
        {
            var productQuery = from p in this._productRepository.TableNoTracking
                               where !p.Deleted
                               select p;

            if (!showHidden)
            {
                productQuery = from p in this._productRepository.TableNoTracking
                               where p.Published && (p.StockQuantity > 0 || p.ProductExtra.IsShippingFromManufacturer)
                                && !p.DisableBuyButton && !p.CallForPrice
                               select p;
            }

            if (!_catalogSettings.IgnoreStoreLimitations)
            {
                productQuery = productQuery.Join(this._storeMappingRepository.TableNoTracking, a => a.Id, b => b.EntityId, (a, b) => new { Product = a, StoreMapping = b })
                            .Where(c => c.StoreMapping.StoreId == _storeContext.CurrentStore.Id && c.StoreMapping.EntityName == "Product" || !c.Product.LimitedToStores)
                            .Select(d => d.Product);
            }

            var startDate = DateTime.UtcNow.AddDays(-90);
            var products = (from bsp in this.bestSellerProductRepository.TableNoTracking
                                .Where(bs => bs.LastOrderDate >= startDate)
                           join p in productQuery on bsp.ProductId equals p.Id
                           orderby bsp.TotalQty descending
                           select p).Take(20);

            return products.OrderBy(m => Guid.NewGuid()).Take(takeCount).ToList();
        }
    }
}
