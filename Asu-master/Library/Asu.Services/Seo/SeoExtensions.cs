...
namespace Asu.Services.Seo
{
    public static class SeoExtensions
    {
        ...
        /// <param name="ensureTwoPublishedLanguages">A value indicating whether to ensure that we have at least two published languages; otherwise, load only default value</param>
        /// <returns>Search engine  name (slug)</returns>
        public static string GetSeName(int entityId, int additionalEntityId, string entityName, int languageId, bool returnDefaultValue = true, bool ensureTwoPublishedLanguages = true)
        {
            string result = string.Empty;
            ...
            if (languageId > 0)
            {
                ...
                if (loadLocalizedValue)
                {
                    result = urlRecordService.GetActiveSlug(entityId, additionalEntityId, entityName, languageId);
                }
            }
            //set default value if required
            if (String.IsNullOrEmpty(result) && returnDefaultValue)
            {
                result = urlRecordService.GetActiveSlug(entityId, additionalEntityId, entityName, 0);
            }
