...
namespace Asu.Services.Media
{
    ...
    public partial class PictureService : IPictureService
    {
        ...
        private readonly IRepository<AdditionalImage> _additionalImageRepository;
            ...
            IStoreContext storeContext)
        {
            ...
            this._additionalImageRepository = additionalImageRepository;
...
#if DEBUG
            string url = $"{this._webHelper.GetStoreImagesLocation()}content/images/{fileName}";
        ...
        protected virtual string GetThumbUrl(string thumbFileName, string storeLocation = null)
        {
            storeLocation = !String.IsNullOrEmpty(storeLocation)
                                    ? storeLocation
                                    : _webHelper.GetStoreImagesLocation();
            ...
            string storeLocation = null)
        {
            ...
            switch (defaultPictureType)
            {
                case PictureType.Entity:
                    defaultImageFileName = _settingService.GetSettingByKey("Media.DefaultImageName", $"{this._storeContext.CurrentStore.GetDefaultPictureNameWithoutExtension(storeLocation)}.gif");
                    break;
        ...
        {
            var query = from img in this._additionalImageRepository.Table
        ...
        public virtual IQueryable<Picture> GetPicturesByIds(int[] pictureIds)
        {
            if (pictureIds.Count() == 0)
                return null;

            return _pictureRepository.TableNoTracking.Where(x=> pictureIds.Contains(x.Id));
        }
        ...
        protected virtual Size CalculateWidthHeightDimensions(Size originalSize, int maxWidth = 0, int maxHeight = 0, bool ensureSizePositive = true)
        {
            var newSize = new Size();
        ...
        public virtual string GetWidthHeightDefaultPictureUrl(int maxWidth = 0, int maxHeight = 0, PictureType defaultPictureType = PictureType.Entity,
            string storeLocation = null)
        {
            string defaultImageFileName;
            switch (defaultPictureType)
            {
                case PictureType.Entity:
                    defaultImageFileName = _settingService.GetSettingByKey("Media.DefaultImageName", $"{this._storeContext.CurrentStore.GetDefaultPictureNameWithoutExtension()}.gif");
                    break;
                case PictureType.Avatar:
                    defaultImageFileName = _settingService.GetSettingByKey("Media.Customer.DefaultAvatarImageName", "default-avatar.jpg");
                    break;
            ...
            {
                string fileExtension = Path.GetExtension(filePath);
                string thumbFileName = string.Format("{0}_{1}_{2}{3}",
                    Path.GetFileNameWithoutExtension(filePath),
                    maxWidth,
                    maxHeight,
                    fileExtension);
        ...
        public virtual string GetWidthHeightPictureUrl(int pictureId, int maxWidth = 0, int maxHeight = 0, bool showDefaultPicture = true,
            string storeLocation = null, PictureType defaultPictureType = PictureType.Entity, bool useSeoName = true)
        {
            var picture = GetPictureById(pictureId);
            return GetWidthHeightPictureUrl(picture, maxWidth, maxHeight, showDefaultPicture, storeLocation, defaultPictureType, useSeoName);
        }

        public virtual string GetWidthHeightPictureUrl(Picture picture, int maxWidth = 0, int maxHeight = 0, bool showDefaultPicture = true,
            string storeLocation = null, PictureType defaultPictureType = PictureType.Entity, bool useSeoName = true)
        {
            string url = string.Empty;
            byte[] pictureBinary = null;
            ...
            lock (s_lock)
            {
                ...
                {
                    thumbFileName = !String.IsNullOrEmpty(seoFileName) ?
                        string.Format("{0}_{1}_{2}_{3}.{4}", picture.Id.ToString("0000000"), seoFileName, maxWidth, maxHeight, lastPart) :
                        string.Format("{0}_{1}_{2}.{3}", picture.Id.ToString("0000000"), maxWidth, maxHeight, lastPart);
                    var thumbFilePath = GetThumbLocalPath(thumbFileName);
                    if (!File.Exists(thumbFilePath))
                    {
                        using (var stream = new MemoryStream(pictureBinary))
        ...
        {
            using (var image = new Bitmap(new MemoryStream(bytes)))
            {
                ...
                var newSize = this.CalculateWidthHeightDimensions(image.Size, width, height);
                var output = new MemoryStream();
                ImageBuilder.Current.Build(image, output, new ResizeSettings
                {
                    Width = newSize.Width,
                    Height = newSize.Height,
                    Scale = ScaleMode.Both,
                    Quality = this._mediaSettings.DefaultImageQuality
                });
