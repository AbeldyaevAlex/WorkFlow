...
namespace Asu.Services.Messages
{
    ...
    public partial interface IWorkflowMessageService
    {
        ...
        /// <summary>
        /// Sends a shipment for manual order to a customer
        /// </summary>
        /// <param name="crmOrder"></param>
        /// <param name="crmShipment"></param>
        /// <param name="languageId"></param>
        /// <returns></returns>
        int SendManualOrderShipment(CrmSalesOrder crmOrder, CrmShipment crmShipment, int languageId);

        /// <summary>
        /// Sends a delivered notification for manual order to a customer
        /// </summary>
        /// <param name="crmOrder"></param>
        /// <param name="crmShipment"></param>
        /// <param name="languageId"></param>
        /// <returns></returns>
        int SendManualOrderDelivered(CrmSalesOrder crmOrder, CrmShipment crmShipment, int languageId);
        ...
        int SendManualOrderCancelledCustomerNotification(CrmSalesOrder order, int languageId);

        #endregion

        #region WC

        int SendCouponNotification(string emailAddress, int languageId);
        int SendShipmentSentCustomerNotification(Shipment shipment, int languageId, bool useBcc, bool askToLeaveReview);
        int SendOrderWithRebatesCustomerNotification(OrderWithRebates orderWithRebates, int languageId);
        int SendProductReviewCustomerNotification(OrderProductToReview orderProductToReview, int languageId);
        int SendOrderShipmentEtaNotification(OrderShipmentEta orderShipmentEta, int languageId);
        int SendReturnRequestHelpSupportNotification(string orderNumber, string fullname, string message, string marketplace, string email, string phone, string toEmail, int languageId = 1);

        int SendSalesQuoteCustomerNotification(SalesQuote quote, int languageId = 0);

        int SendUpdateOrderDelayedOutstockLowestCostCustomerNotification(Order order, int shipIndays, int languageId = 0);

        int SendOldUpdateOrderDelayedOutstockLowestCostCustomerNotification(Order order, int languageId = 0);

        int SendOrderReviewNotification(Order order, int languageId = 0);

        int SendEbayOrderDeliveryNotification(CrmSalesOrder order, int languageId = 0);

        int SendCustomShipmentDeliveredCustomerNotification(Shipment shipment, int languageId = 0);

        int SendOosWithEtaCustomerNotification(Order order, int languageId = 0);

        int SendUpdatedEtaCustomerNotification(Order order, DateTime eta, int languageId = 0);

        int SendUpdatedEtaNoDateCustomerNotification(Order order, int languageId = 0);
