...
namespace Asu.Services.Tax
{
    ...
    public partial class TaxService : ITaxService
    {
        ...
        // WC. Added to support multiple tax rates
        private void GetMaxTaxRate(Product product, int taxCategoryId, Customer customer, out decimal taxRate, out bool isTaxable)
        {
            taxRate = decimal.Zero;
            isTaxable = true;

            var taxProviders = this.LoadAllTaxProviders();
            foreach (var taxProvider in taxProviders)
            {
                decimal providerTaxRate;
                bool providerIsTaxable;

                this.GetTaxRate(taxProvider, product, taxCategoryId, customer, out providerTaxRate, out providerIsTaxable);

                if (providerTaxRate > taxRate)
                {
                    taxRate = providerTaxRate;
                    isTaxable = providerIsTaxable;
                }
            }
        }

        // WC.
        private void GetTaxRate(ITaxProvider taxProvider, Product product, int taxCategoryId, Customer customer, out decimal taxRate, out bool isTaxable)
        {
            taxRate = decimal.Zero;
            isTaxable = true;

            //tax exempt
            if (this.IsTaxExempt(product, customer))
            {
                isTaxable = false;
            }

            //tax request
            var calculateTaxRequest = this.CreateCalculateTaxRequest(product, taxCategoryId, customer);
            ...
            //get tax rate
            var calculateTaxResult = taxProvider.GetTaxRate(calculateTaxRequest);
            if (calculateTaxResult.Success)
            {
                //ensure that tax is equal or greater than zero
                if (calculateTaxResult.TaxRate < decimal.Zero)
            ...
            bool priceIncludesTax, out decimal taxRate)
        {
            ...
            // WC. Added to support multiple tax providers and to select max tax from them (possible a different way to select a best tax provider)
            GetMaxTaxRate(product, taxCategoryId, customer, out taxRate, out isTaxable);
        ...
        public virtual decimal GetShippingPrice(decimal price, bool includingTax, Customer customer, out decimal taxRate)
        {
            ...
            if (!_taxSettings.ShippingIsTaxable || taxableShippingStates.All(m => m != customer?.ShippingAddress?.StateProvinceId))
            {
                return price;
            }
