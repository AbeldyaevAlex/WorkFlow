...
namespace Asu.Services.Orders
{
    ...
    public partial class OrderService : IOrderService
    {
        ...
        private readonly IRepository<SignifydDeviceFingerprints> _SignifydDeviceFingerprintsRepository;
        ...
        private readonly IRepository<CrmSalesOrderCancelReason> crmSalesOrderCancelReasonRepository;
        private readonly IStoreContext storeContext;
        //private readonly IEventPublisher _eventPublisher;
        private readonly IRepository<GdprOrder> gdprOrderRepository;
        private readonly IRepository<CrmSalesOrderLine> crmOrderLineRepository;
        private readonly IRepository<CrmSalesOrder> salesOrderRepository;
        ...
        /// <param name="klaviyoService">Klaviyo service</param>
        public OrderService(IRepository<Order> orderRepository,
            IRepository<OrderItem> orderItemRepository,
            IRepository<OrderNote> orderNoteRepository,
            IRepository<SignifydDeviceFingerprints> SignifydDeviceFingerprintsRepository,
            IRepository<Product> productRepository,
            IRepository<RecurringPayment> recurringPaymentRepository,
            IRepository<Customer> customerRepository,
            IRepository<CrmSalesOrder> crmOrderRepository,
            IRepository<ThubOrderItem> thubOrderItemRepository,
            IRepository<CrmSalesOrderCancelReason> crmSalesOrderCancelReasonRepository,
            IStoreContext storeContext,
            /*IEventPublisher eventPublisher*/
            IRepository<GdprOrder> gdprOrderRepository,
            IRepository<CrmSalesOrderLine> crmOrderLineRepository,
            IRepository<CrmSalesOrder> salesOrderRepository)
        {
            this._orderRepository = orderRepository;
            this._orderItemRepository = orderItemRepository;
            this._orderNoteRepository = orderNoteRepository;
            this._SignifydDeviceFingerprintsRepository = SignifydDeviceFingerprintsRepository;
            ...
            this.crmSalesOrderCancelReasonRepository = crmSalesOrderCancelReasonRepository;
            this.storeContext = storeContext;
            //this._eventPublisher = eventPublisher;
            this.gdprOrderRepository = gdprOrderRepository;
            this.crmOrderLineRepository = crmOrderLineRepository;
            ...
            if (warehouseId > 0)
            {
                ...
                query = query
                    ...
                    .Any(orderItem =>
                        ...
                        ((int)productService.GetInventoryManageMethod(orderItem.Product, this.storeContext.CurrentStore.Id) == manageStockInventoryMethodId && 
        ...
        #region WC
        public virtual IPagedList<Order> SearchDeletedOrders(int storeId = 0, int customerId = 0, DateTime? createdFromUtc = null,
            DateTime? createdToUtc = null, OrderStatus? os = null, PaymentStatus? ps = null, ShippingStatus? ss = null,
            int pageIndex = 0, int pageSize = int.MaxValue)
        {
            int? orderStatusId = null;
            if (os.HasValue)
        ...
        public virtual Order[] GetOrdersByAuthorizationTransactionIdAndPaymentMethod(string authorizationTransactionId,
        ...
        /// </summary>
        /// <param name="order">Order</param>
        public virtual void InsertSignifydDeviceFingerprints(SignifydDeviceFingerprints signifydDeviceFingerprints)
        {
            if (signifydDeviceFingerprints == null)
                throw new ArgumentNullException("SignifydDeviceFingerprints");

            _SignifydDeviceFingerprintsRepository.Insert(signifydDeviceFingerprints);
        ...
        /// <summary>
        /// Gets a THUB order item
        /// </summary>
        /// <param name="orderItemId">THUB order item identifier</param>
        /// <returns>THUB order item</returns>
        public virtual ThubOrderItem GetThubOrderItemById(long orderItemId)
        {
            if (orderItemId == 0)
            {
                return null;
            ...
            return this.thubOrderItemRepository.GetById(orderItemId);
        }

        /// <summary>
        /// Gets a CRM order line
        /// </summary>
        /// <param name="lineId">CRM order line identifier</param>
        /// <returns>CRM order line</returns>
        public virtual CrmSalesOrderLine GetOrderLineById(int lineId)
        {
            if (lineId == 0)
            {
                return null;
            ...
            return this.crmOrderLineRepository.GetById(lineId);
        }
        ...
        public CrmSalesOrder GetCrmOrder(int id)
        {
        ...
        public int? GetCrmOrderIdByReference(string orderReference, int channelId)
        {
        ...
        public virtual IList<Order> GetOrdersByTimeRange(DateTime start, DateTime? end = null, int? storeId = null)
        {
            var query = from o in this._orderRepository.Table
                        where !o.Deleted && o.CreatedOnUtc >= start
                        select o;

            if (end.HasValue)
            {
                query = from o in query
                        where o.CreatedOnUtc <= end.Value
                        select o;
            }
        ...
        public virtual IList<CrmSalesOrder> GetSalesOrdersByTimeRange(DateTime start, DateTime? end = null, Channel? channel = null)
        {
            var query = from o in this.salesOrderRepository.Table
                        where o.CreatedOn >= start
                        select o;

            if (channel.HasValue)
            {
                query = query.Where(o => o.ChannelId == (int)channel.Value);
            }

            if (end.HasValue)
            {
                query = from o in query
                        where o.CreatedOn <= end.Value
                        select o;
            }
        ...
        public bool IsGdprOrder(int orderId)
        {
            var query = from m in this.gdprOrderRepository.TableNoTracking
        ...
        {
            var query = from o in this._orderRepository.TableNoTracking
        ...
        public CrmSalesOrderCancelReason GetCancellationReason(Order order)
        {
            return (from a in this.crmSalesOrderCancelReasonRepository.TableNoTracking
                    where a.NopOrderId == order.Id
                    select a).ToList().FirstOrDefault();
        }

        public CrmSalesOrderCancelReason GetCancellationReason(CrmSalesOrder order)
        {
            return (from a in this.crmSalesOrderCancelReasonRepository.TableNoTracking
                    where a.SalesOrderId == order.Id
                    select a).ToList().FirstOrDefault();
        }

        public CrmSalesOrderCancelReason GetCancellationCustomerReason(Order order)
        {
            return (from a in this.crmSalesOrderCancelReasonRepository.TableNoTracking
                    where a.NopOrderId == order.Id
                    select a).ToList().FirstOrDefault();
        ...
        public CrmSalesOrderCancelReason GetCancellationCustomerReason(CrmSalesOrder order)
        {
            return  (from a in this.crmSalesOrderCancelReasonRepository.TableNoTracking
                    where a.SalesOrderId == order.Id
                    select a).ToList().FirstOrDefault();
        ...
        public Channel GetChannel(NopStore store)
        {
            Channel channel = Channel.Autoplicity;
            switch (store)
            {
                case NopStore.Autoplicity:
                    channel = Channel.Autoplicity;
                    break;
                case NopStore.Boatplicity:
                    channel = Channel.Boatplicity;
                    break;
                case NopStore.Thmotorsports:
                    channel = Channel.Thmotorsports;
                    break;
                case NopStore.Cycleplicity:
                    channel = Channel.Cycleplicity;
                    break;
                case NopStore.DirtEverywhere:
                    channel = Channel.DirtEverywhere;
                    break;
            }
