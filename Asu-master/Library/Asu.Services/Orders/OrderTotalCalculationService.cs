...
namespace Asu.Services.Orders
{
    ...
    public partial class OrderTotalCalculationService : IOrderTotalCalculationService
    {
        ...
        protected virtual decimal GetOrderSubtotalDiscount(Customer customer,
            decimal orderSubTotal, IList<ShoppingCartItem> cart, out Discount appliedDiscount)
        {
            ...
            // decimal subTotalRecommendedProductsDiscount = GetSubTotalRecommendedProductsDiscount(cart);

            return discountAmount;// + subTotalRecommendedProductsDiscount;
        }
        ...
        #region RecommendationCalculation

        protected decimal GetSubTotalRecommendedProductsDiscount(IList<ShoppingCartItem> cart)
        {
            var productExtras = _orderReportService.GetProductExtrasByProductRecommendations(this._storeContext.CurrentStore.Id,
                cart.Select(i => i.ProductId).ToArray()).ToList();

            decimal subTotalRecommendedProductsDiscount = decimal.Zero;

            foreach (var item in cart)
            {
                var recommendedExtra = productExtras.Any()
                    ? productExtras.FirstOrDefault(p =>
                        p.Product.Id == item.ProductId && p.BundleDiscount != null)
                    : null;

                var parentProductOfThisRecommended =
                    _orderReportService.GetParentProductIdsOfProductRecommendations(this._storeContext.CurrentStore.Id,
                        item.ProductId);

                int currentItemIndex = cart.IndexOf(item);
                int parentItemIndex = cart.Count + 1;

                if (parentProductOfThisRecommended.Any())
                {
                    foreach (var r in parentProductOfThisRecommended)
                    {
                        if(!cart.Select(p => p.ProductId).Contains(r))
                            continue;
                        var parentIndex = cart.IndexOf(cart.First(p => p.ProductId == r));
                        if (parentIndex < parentItemIndex)
                            parentItemIndex = parentIndex;
                    }
                }

                if (recommendedExtra != null && recommendedExtra.BundleDiscount != null && parentItemIndex < currentItemIndex)
                {
                    subTotalRecommendedProductsDiscount +=
                        item.Product.Price * (decimal) recommendedExtra.BundleDiscount;
                }

            }

            return Math.Round(subTotalRecommendedProductsDiscount, 2);
        }
            ...
            out SortedDictionary<decimal, decimal> taxRates)
        {
            ...
            decimal discountAmountExclTax = GetOrderSubtotalDiscount(customer, subTotalExclTaxWithoutDiscount, cart, out appliedDiscount);
        ...
        {
            var isFreeShipping = this.IsFreeShipping(cart);

            return isFreeShipping ? decimal.Zero : cart.Where(sci => sci.IsShipEnabled && !this._shippingService.IsFreeShipping(sci.Product, this._storeContext.CurrentStore.Id)).Sum(sci => sci.AdditionalShippingCharge);
        }
        ...
        public virtual bool IsFreeShipping(IList<ShoppingCartItem> cart)
        {
            ...
            var allItemsAreFreeShipping = cart.All(sc => !sc.IsShipEnabled || this._shippingService.IsFreeShipping(sc.Product, this._storeContext.CurrentStore.Id));
        ...
        public virtual decimal AdjustShippingRate(decimal shippingRate, IList<ShoppingCartItem> cart, out decimal discountAmount, out Discount appliedDiscount)
        {
            appliedDiscount = null;
            discountAmount = 0;
        ...
        public virtual decimal? GetShoppingCartShippingTotal(IList<ShoppingCartItem> cart, bool includingTax, out decimal taxRate, out decimal discountAmount,
            ...
            out SortedDictionary<decimal, decimal> taxRates, bool usePaymentMethodAdditionalFee = true)
        {
            ...
            var taxableShippingStates = this._taxSettings.TaxableShippingStates.Split(',').Select(m => int.Parse(m));
            if (_taxSettings.ShippingIsTaxable && taxableShippingStates.Any(m => m == customer?.ShippingAddress?.StateProvinceId))
            {
                decimal taxRate;
            ...
            bool ignoreRewardPonts = false, bool usePaymentMethodAdditionalFee = true)
        {
            ...
            {
                var adminShipping = _workContext.CurrentCustomer.GetAttribute<decimal?>("AdminShipping", _storeContext.CurrentStore.Id);
                if (adminShipping.HasValue)
                {
                    shoppingCartShipping = adminShipping;
                }
            }
            ...
            if (orderSubTotalDiscountAmount > decimal.Zero)
                discountAmount = orderSubTotalDiscountAmount;
