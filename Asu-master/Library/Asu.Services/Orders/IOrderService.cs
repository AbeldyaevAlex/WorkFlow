...
namespace Asu.Services.Orders
{
    ...
    public partial interface IOrderService
    {
        ...
        IPagedList<Order> SearchDeletedOrders(int storeId = 0, int customerId = 0, DateTime? createdFromUtc = null,
            DateTime? createdToUtc = null, OrderStatus? os = null, PaymentStatus? ps = null, ShippingStatus? ss = null,
            int pageIndex = 0, int pageSize = int.MaxValue);

        Order[] GetOrdersByAuthorizationTransactionIdAndPaymentMethod(string authorizationTransactionId, string paymentMethodSystemName);

        /// <summary>
        /// Gets a CRM order by id
        /// </summary>
        /// <param name="id">The CRM order id</param>
        CrmSalesOrder GetCrmOrder(int id);

        /// <summary>
        /// Gets a CRM order Id by order reference
        /// </summary>
        /// <param name="orderReference">The order reference</param>
        /// <param name="channelId">THUB channel Id</param>
        int? GetCrmOrderIdByReference(string orderReference, int channelId);
        ...
        /// <summary>
        /// Inserts an SignifydDeviceFingerprints
        /// </summary>
        /// <param name="SignifydDeviceFingerprints">SignifydDeviceFingerprints</param>
        void InsertSignifydDeviceFingerprints(SignifydDeviceFingerprints SignifydDeviceFingerprints);
        ...
        ThubOrderItem GetThubOrderItemById(long orderItemId);
        ...
        bool IsGdprOrder(int orderId);

        CrmSalesOrderLine GetOrderLineById(int lineId);

        IList<Order> GetOrdersByTimeRange(DateTime start, DateTime? end = null, int? storeId = null);

        IList<CrmSalesOrder> GetSalesOrdersByTimeRange(DateTime start, DateTime? end = null, Channel? channel = null);

        IList<Order> GetOrdersByCustomerId(int customerId);

        Channel GetChannel(NopStore storeId);

        CrmSalesOrderCancelReason GetCancellationReason(Order order);

        CrmSalesOrderCancelReason GetCancellationReason(CrmSalesOrder order);

        CrmSalesOrderCancelReason GetCancellationCustomerReason(Order order);

        CrmSalesOrderCancelReason GetCancellationCustomerReason(CrmSalesOrder order);
    }
}
