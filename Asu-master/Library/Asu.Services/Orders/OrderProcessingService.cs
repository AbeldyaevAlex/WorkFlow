...
namespace Asu.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
            ...
            IKlaviyoService klaviyoService)
        {
            ...
            this._klaviyoService = klaviyoService;
        ...
        protected virtual void SetOrderStatus(Order order, OrderStatus os, bool notifyCustomer)
        {
            ...
            //WC. Commented to improve performance
        ...
        public virtual PlaceOrderResult PlaceOrder(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            try
            {
                ...
                var isAdmin = customer.IsAdmin();   // WC
                ...
                //checkout attributes
                // NOTE(3/2/2021): commented to save order items(CRM import issue)
                string checkoutAttributeDescription = "OrderItems"; 
                ...
                if (!processPaymentRequest.IsRecurringPayment)
                {
                    ...
                    var warnings = _shoppingCartService.GetShoppingCartWarnings(cart,
                        string.Empty, // NOTE(3/2/2021): checkoutAttributesXml,
                        true);
                ...
                {
                    processPaymentRequest.PaymentMethodSystemName = string.Empty;
                }
                ...
                var isRetrurnExtensionApplied = this.returnExtensionService.IsShowReturnExtension() && this.returnExtensionService.IsReturnExtensionApplied(cart);
                if (processPaymentResult.Success)
                {
                    ...
                    //using (var transaction = new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = System.Transactions.IsolationLevel.Serializable }))
                    {
                        ...
                        #region WC - Admin Shipping override

                        if (isAdmin)
                        {
                            var adminShipping = _workContext.CurrentCustomer.GetAttribute<decimal?>("AdminShipping", 1);
                            if (adminShipping.HasValue)
                            {
                                orderShippingTotalInclTax = adminShipping;
                                orderShippingTotalExclTax = adminShipping;
                            }
                        }

                        #endregion

                        #region CRM Order Import Issue 

                        // NOTE(3/2/2021): 
                        var isQuote = this.salesQuoteService.GetQuoteIdFromCookie() > 0;
                        var items = cart.Select(sci =>
                        {
                            decimal taxRate;
                            Discount scDiscount;
                            decimal discountAmount;
                            decimal scUnitPrice = _priceCalculationService.GetUnitPrice(sci);

                            #region WC - Admin Price override. Unit price override
                            if (isAdmin || isQuote)
                            {
                                try
                                {
                                    var adminProductPrice = customer.GetAttribute<decimal?>(string.Format("AdminProductPrice-{0}", sci.ProductId), _genericAttributeService, this.storeContext.CurrentStore.Id);
                                    if (adminProductPrice.HasValue)
                                    {
                                        scUnitPrice = adminProductPrice.Value;
                                    }
                                }
                                catch (Exception)
                                {
                            ...
                            decimal scSubTotal = _priceCalculationService.GetSubTotal(sci, true, out discountAmount, out scDiscount);
                            decimal scUnitPriceInclTax = _taxService.GetProductPrice(sci.Product, scUnitPrice, true, customer, out taxRate);
                            decimal scUnitPriceExclTax = _taxService.GetProductPrice(sci.Product, scUnitPrice, false, customer, out taxRate);
                            decimal scSubTotalInclTax = _taxService.GetProductPrice(sci.Product, scSubTotal, true, customer, out taxRate);
                            decimal scSubTotalExclTax = _taxService.GetProductPrice(sci.Product, scSubTotal, false, customer, out taxRate);

                            decimal discountAmountInclTax = _taxService.GetProductPrice(sci.Product, discountAmount, true, customer, out taxRate);
                            decimal discountAmountExclTax = _taxService.GetProductPrice(sci.Product, discountAmount, false, customer, out taxRate);
                            if (scDiscount != null && !appliedDiscounts.ContainsDiscount(scDiscount))
                                appliedDiscounts.Add(scDiscount);

                            //attributes
                            // NOTE(3/2/2021): commented for performance purposes
                            // string attributeDescription = _productAttributeFormatter.FormatAttributes(sc.Product, sc.AttributesXml, customer);

                            var itemWeight = _shippingService.GetShoppingCartItemWeight(sci);
                            ...
                            return new OrderItem
                            {
                                ...
                                OriginalProductCost = _priceCalculationService.GetProductCost(sci.Product, sci.AttributesXml),
                                AttributeDescription = string.Empty, // NOTE(3/2/2021): attributeDescription,
                        ...
                        using (var sww = new StringWriter())
                        {
                            using (XmlWriter writer = XmlWriter.Create(sww))
                            {
                                ...
                                checkoutAttributesXml = sww.ToString(); 
                            }
                        }
                        ...
                        var order = new Order
                        {
                            ...
                            CardExpirationMonth = processPaymentResult.AllowStoringCreditCardNumber ? _encryptionService.EncryptText(processPaymentRequest.CreditCardExpireMonth.ToString("00")) : string.Empty,
                        ...
                        #region WC : Replacing of logic of commented above lines in the Order object initialization
                        /*order.CardNumber = processPaymentResult.AllowStoringCreditCardNumber ? processPaymentRequest.CreditCardNumber : string.Empty;
                        order.CardExpirationMonth = processPaymentResult.AllowStoringCreditCardNumber ? processPaymentRequest.CreditCardExpireMonth.ToString("00") : string.Empty;
                        order.CardExpirationYear = processPaymentResult.AllowStoringCreditCardNumber ? processPaymentRequest.CreditCardExpireYear.ToString() : string.Empty;*/
                        #endregion
                        ...
                        {
                            this._shippingService.InsertOriginalShippingRate(new OriginalShippingRate
                            {
                                OrderId = order.Id,
                                Value = originalShippingRate.Value,
                            });
                        }
                        var SignifydDeviceFingerprints = new SignifydDeviceFingerprints
                        {
                            OrderId = order.Id,
                        ...
                        try
                        {
                            _orderService.InsertSignifydDeviceFingerprints(SignifydDeviceFingerprints);
                        }
                        catch (Exception e)
                        {
                        ...
                        /*#region WC Kount Service

                        if (!isAdmin)
                        {
                            this._customService.CallKountService(order.Id, shippingMethodName, customer, processPaymentRequest, processPaymentResult);
                        }
                        ...
                        if (!processPaymentRequest.IsRecurringPayment)
                        {
                            ...
                            foreach (var sc in cart)
                            {
                                ...
                                //    try
                                //    {
                                //        var adminProductPrice = customer.GetAttribute<decimal?>(string.Format("AdminProductPrice-{0}", sc.ProductId), _genericAttributeService, this.storeContext.CurrentStore.Id);
                                //        if (adminProductPrice.HasValue)
                                //        {
                                //            scUnitPrice = adminProductPrice.Value;
                                //        }
                                ...
                                var orderItem = items.FirstOrDefault(i => i.ProductId == sc.ProductId);
                                ...
                                if (sc.Product.IsGiftCard)
                                {
                                    ...
                                    for (int i = 0; i < sc.Quantity; i++)
                                    {
                                        var gc = new GiftCard
                                        {
                                            ...
                                            Amount = orderItem.UnitPriceExclTax, // NOTE(3/2/2021)
                            ...
                            // Validate OrderTotal
                            if (order.OrderTotal != order.OrderSubtotalExclTax + order.OrderShippingExclTax + order.OrderTax - order.OrderDiscount)
                            {
                                order.OrderDiscount = Math.Round(order.OrderTotal - (order.OrderSubtotalExclTax + order.OrderShippingExclTax + order.OrderTax), 2);
                                _orderService.UpdateOrder(order);
                            }
                        ...
                        else
                        {
                            ...
                            //if (order.OrderTotal != order.OrderSubtotalInclTax + order.OrderShippingInclTax - order.OrderDiscount)
                            if (order.OrderTotal != order.OrderSubtotalExclTax + order.OrderShippingExclTax + order.OrderTax - order.OrderDiscount)
                            {
                                //order.OrderTotal = Math.Round(order.OrderSubtotalExclTax + order.OrderShippingExclTax + order.OrderTax - order.OrderDiscount, 2);
                                order.OrderDiscount = Math.Round(order.OrderTotal - (order.OrderSubtotalExclTax + order.OrderShippingExclTax + order.OrderTax), 2);
                                //order.OrderTax = Math.Round(order.OrderSubtotalInclTax + order.OrderShippingInclTax - order.OrderDiscount, 2);
                                _orderService.UpdateOrder(order);
                            }
                        }

                        if (order.OrderStatus == OrderStatus.Pending && items.Count == order.OrderItems.Count)
                        {
                        ...
                        {
                            this.warrantyService.SaveInsurance(order.Id, order.OrderItems);
                        }

                        //if (isRetrurnExtensionApplied)
                        //{
                        //    this.warrantyService.SaveReturnExtension(order.Id, order.OrderItems);
                        //}

                        var storeId = this.storeContext.CurrentStore.Id;
                        if (storeId == (int)NopStore.Autoplicity || storeId == (int)NopStore.Thmotorsports)
                        {
                        ...
                        if (order.OrderItems.Any(i => i.Product.ProductExtra.IsShippingFromManufacturer))
                        {
                            int backorderCustomerNotificationQueuedEmailId = this._workflowMessageService.SendOosWithEtaCustomerNotification(order, order.CustomerLanguageId);
                        }

#if DEBUG
#else
    this._klaviyoService.TrackPlacedOrderActivity(order);
#endif
                        ...
                        if (isAdmin)
                        {
                            try
                            {
                                var adminShippingAttribute = _workContext.CurrentCustomer.GetAttribute("AdminShipping", _genericAttributeService, 1);
                                if (adminShippingAttribute != null)
                                {
                                    _genericAttributeService.DeleteAttribute(adminShippingAttribute);
                                }

                                foreach (var item in order.OrderItems)
                                {
                                    var adminPriceAttribute = _workContext.CurrentCustomer.GetAttribute(string.Format("AdminProductPrice-{0}", item.ProductId), _genericAttributeService, 1);
                                    if (adminPriceAttribute != null)
                                    {
                                        _genericAttributeService.DeleteAttribute(adminPriceAttribute);
                                    }
                                }
                            }
                            catch (Exception)
                ...
                else
                {
                    ...
                    if (!isAdmin)
                    {
                        this._customService.CallKountService(null, shippingMethodName, customer, processPaymentRequest, processPaymentResult);
                    }
...
#if DEBUG
#else
    this._klaviyoService.TrackOrderShipped(order);
#endif
            _orderService.UpdateOrder(order);
...
#region WC (Send BCC for "ShipmentSent.CustomerNotification" email only if ship date - order date < 48h)

                int queuedEmailId = _workflowMessageService.SendShipmentSentCustomerNotification(shipment, order.CustomerLanguageId, 
                    useBcc: (DateTime.UtcNow - order.CreatedOnUtc).TotalHours < 48, askToLeaveReview: (DateTime.UtcNow - order.CreatedOnUtc).TotalHours < 48);

#endregion

                if (queuedEmailId > 0)
                {
                    shipment.IsEmailSent = true;
                    shipment.ShippedDateUtc = DateTime.UtcNow;
                    _shipmentService.UpdateShipment(shipment);
...
#if DEBUG
#else
    this._klaviyoService.TrackOrderDelivered(order);
#endif
            _orderService.UpdateOrder(order);
            ...
            if (notifyCustomer)
            {
                ...
                int queuedEmailId = _workflowMessageService.SendCustomShipmentDeliveredCustomerNotification(shipment, order.CustomerLanguageId);
                if (queuedEmailId > 0)
                {
                    order.OrderNotes.Add(new OrderNote
                    {
        ...
        public virtual bool IsReturnRequestAllowed(Order order)
        {
            ...
            bool numberOfDaysReturnRequestAvailableValid;
