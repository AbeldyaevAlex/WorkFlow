...
namespace Asu.Services.Orders
{
    ...
    public partial class OrderReportService : IOrderReportService
    {
        ...
        private readonly IRepository<ProductRecommendation> _productRecommendation;
        private readonly IRepository<ProductExtra> _productExtra;
        ...
        public OrderReportService(IRepository<Order> orderRepository,
            ...
            IRepository<ProductRecommendation> productRecommendation,
            IRepository<ProductExtra> productExtra,
            ...
            IStoreMappingService storeMappingService)
        {
            ...
            this._productRecommendation = productRecommendation;
            this._productExtra = productExtra;
        ...
        public virtual int[] GetAlsoPurchasedProductsIds(int storeId, int productId,
            int recordsToReturn = 20, bool showHidden = false)
        {
            if (productId == 0)
            ...
            var orderIdQuery = from orderItem in _orderItemRepository.Table
                          where orderItem.ProductId == productId
                          select orderItem.OrderId;

            var orderItemProductQuery = from orderItem in _orderItemRepository.Table
                         join p in _productRepository.Table on orderItem.ProductId equals p.Id                        
                         where orderIdQuery.Contains(orderItem.OrderId)                        
                         && p.Id != productId && (showHidden || p.Published)
                         && !orderItem.Order.Deleted && !p.Deleted
                         ...
                         select new { OrderItem = orderItem, Product = p };


            var soldProducts = orderItemProductQuery
                .GroupBy(m => m.Product.Id)
                .Select(g => new { ProductId = g.Key, ProductsPurchased = g.Sum(x => x.OrderItem.Quantity) })
                .OrderByDescending(p => p.ProductsPurchased);

            var bestSellers = soldProducts
        ...
        {
            return _productExtra.Table.Where(p => productsIds.Contains(p.Product.Id)).Select(p => p).ToList();
        }
        ...
        public virtual List<ProductRecommendation> GetProductsRecommendations(int storeId, int productId, bool showHidden = false)
        {
            if (productId == 0)
                throw new ArgumentException("Product ID is not specified");
            //this inner query should retrieve all orders that contains a specified product ID
            var orderIdQuery = from productRecommend in _productRecommendation.Table
                               where productRecommend.ParentProductId == productId
                               select productRecommend.ProductId;

            var orderItemProductQuery = from productRecommend in _productRecommendation.Table
        ...
        /// <param name="showHidden">A value indicating whether to show hidden records</param>
        /// <returns>Products</returns>
        public virtual int[] GetParentProductIdsOfProductRecommendations(int storeId, int productId, bool showHidden = false)
        {
            if (productId == 0)
                throw new ArgumentException("Product ID is not specified");
            //this inner query should retrieve all orders that contains a specified product ID
            ...
            var parentProducts = orderIdQuery.ToArray().GroupBy(p => p).Select(p => p.First()).ToArray();
